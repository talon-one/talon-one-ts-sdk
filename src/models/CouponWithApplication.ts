/* tslint:disable */
/* eslint-disable */
/**
 * Talon.One API
 * Use the Talon.One API to integrate with your application and to manage applications and campaigns:  - Use the operations in the [Integration API section](#integration-api) are used to integrate with our platform - Use the operation in the [Management API section](#management-api) to manage applications and campaigns.  ## Determining the base URL of the endpoints  The API is available at the same hostname as your Campaign Manager deployment. For example, if you access the Campaign Manager at `https://yourbaseurl.talon.one/`, the URL for the [updateCustomerSessionV2](https://docs.talon.one/integration-api#operation/updateCustomerSessionV2) endpoint is `https://yourbaseurl.talon.one/v2/customer_sessions/{Id}` 
 *
 * The version of the OpenAPI document: 
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime';
import type { LimitConfig } from './LimitConfig';
import {
    LimitConfigFromJSON,
    LimitConfigFromJSONTyped,
    LimitConfigToJSON,
    LimitConfigToJSONTyped,
} from './LimitConfig';

/**
 * 
 * @export
 * @interface CouponWithApplication
 */
export interface CouponWithApplication {
    /**
     * The internal ID of the coupon.
     * @type {number}
     * @memberof CouponWithApplication
     */
    id: number;
    /**
     * The time the coupon was created.
     * @type {Date}
     * @memberof CouponWithApplication
     */
    created: Date;
    /**
     * The ID of the campaign that owns this entity.
     * @type {number}
     * @memberof CouponWithApplication
     */
    campaignId: number;
    /**
     * The coupon code.
     * @type {string}
     * @memberof CouponWithApplication
     */
    value?: string;
    /**
     * The number of times the coupon code can be redeemed. `0` means unlimited redemptions but any campaign usage limits will still apply.
     * 
     * @type {number}
     * @memberof CouponWithApplication
     */
    usageLimit?: number;
    /**
     * The total discount value that the code can give. Typically used to represent a gift card value.
     * 
     * @type {number}
     * @memberof CouponWithApplication
     */
    discountLimit?: number;
    /**
     * The number of reservations that can be made with this coupon code.
     * 
     * @type {number}
     * @memberof CouponWithApplication
     */
    reservationLimit?: number;
    /**
     * Timestamp at which point the coupon becomes valid.
     * @type {Date}
     * @memberof CouponWithApplication
     */
    startDate?: Date;
    /**
     * Expiration date of the coupon. Coupon never expires if this is omitted.
     * @type {Date}
     * @memberof CouponWithApplication
     */
    expiryDate?: Date;
    /**
     * Limits configuration for a coupon. These limits will override the limits
     * set from the campaign.
     * 
     * **Note:** Only usable when creating a single coupon which is not tied to a specific recipient.
     * Only per-profile limits are allowed to be configured.
     * 
     * @type {Array<LimitConfig>}
     * @memberof CouponWithApplication
     */
    limits?: Array<LimitConfig>;
    /**
     * The number of times the coupon has been successfully redeemed.
     * @type {number}
     * @memberof CouponWithApplication
     */
    usageCounter: number;
    /**
     * The amount of discounts given on rules redeeming this coupon. Only usable if a coupon discount budget was set for this coupon.
     * @type {number}
     * @memberof CouponWithApplication
     */
    discountCounter?: number;
    /**
     * The remaining discount this coupon can give.
     * @type {number}
     * @memberof CouponWithApplication
     */
    discountRemainder?: number;
    /**
     * The number of times this coupon has been reserved.
     * @type {number}
     * @memberof CouponWithApplication
     */
    reservationCounter?: number;
    /**
     * Custom attributes associated with this coupon.
     * @type {object}
     * @memberof CouponWithApplication
     */
    attributes?: object;
    /**
     * The integration ID of the referring customer (if any) for whom this coupon was created as an effect.
     * @type {number}
     * @memberof CouponWithApplication
     */
    referralId?: number;
    /**
     * The Integration ID of the customer that is allowed to redeem this coupon.
     * @type {string}
     * @memberof CouponWithApplication
     */
    recipientIntegrationId?: string;
    /**
     * The ID of the Import which created this coupon.
     * @type {number}
     * @memberof CouponWithApplication
     */
    importId?: number;
    /**
     * Defines the reservation type:
     * - `true`: The coupon can be reserved for multiple customers.
     * - `false`: The coupon can be reserved only for one customer. It is a personal code.
     * 
     * @type {boolean}
     * @memberof CouponWithApplication
     */
    reservation?: boolean;
    /**
     * The id of the batch the coupon belongs to.
     * @type {string}
     * @memberof CouponWithApplication
     */
    batchId?: string;
    /**
     * An indication of whether the code can be redeemed only if it has been reserved first.
     * @type {boolean}
     * @memberof CouponWithApplication
     */
    isReservationMandatory?: boolean;
    /**
     * An indication of whether the coupon is implicitly reserved for all customers.
     * @type {boolean}
     * @memberof CouponWithApplication
     */
    implicitlyReserved?: boolean;
    /**
     * The ID of the application.
     * @type {number}
     * @memberof CouponWithApplication
     */
    applicationId: number;
    /**
     * Name of the Application that is connected to the coupon.
     * @type {string}
     * @memberof CouponWithApplication
     */
    applicationName: string;
}

/**
 * Check if a given object implements the CouponWithApplication interface.
 */
export function instanceOfCouponWithApplication(value: object): value is CouponWithApplication {
    if (!('id' in value) || value['id'] === undefined) return false;
    if (!('created' in value) || value['created'] === undefined) return false;
    if (!('campaignId' in value) || value['campaignId'] === undefined) return false;
    if (!('usageCounter' in value) || value['usageCounter'] === undefined) return false;
    if (!('applicationId' in value) || value['applicationId'] === undefined) return false;
    if (!('applicationName' in value) || value['applicationName'] === undefined) return false;
    return true;
}

export function CouponWithApplicationFromJSON(json: any): CouponWithApplication {
    return CouponWithApplicationFromJSONTyped(json, false);
}

export function CouponWithApplicationFromJSONTyped(json: any, ignoreDiscriminator: boolean): CouponWithApplication {
    if (json == null) {
        return json;
    }
    return {
        
        'id': json['id'],
        'created': (new Date(json['created'])),
        'campaignId': json['campaignId'],
        'value': json['value'] == null ? undefined : json['value'],
        'usageLimit': json['usageLimit'] == null ? undefined : json['usageLimit'],
        'discountLimit': json['discountLimit'] == null ? undefined : json['discountLimit'],
        'reservationLimit': json['reservationLimit'] == null ? undefined : json['reservationLimit'],
        'startDate': json['startDate'] == null ? undefined : (new Date(json['startDate'])),
        'expiryDate': json['expiryDate'] == null ? undefined : (new Date(json['expiryDate'])),
        'limits': json['limits'] == null ? undefined : ((json['limits'] as Array<any>).map(LimitConfigFromJSON)),
        'usageCounter': json['usageCounter'],
        'discountCounter': json['discountCounter'] == null ? undefined : json['discountCounter'],
        'discountRemainder': json['discountRemainder'] == null ? undefined : json['discountRemainder'],
        'reservationCounter': json['reservationCounter'] == null ? undefined : json['reservationCounter'],
        'attributes': json['attributes'] == null ? undefined : json['attributes'],
        'referralId': json['referralId'] == null ? undefined : json['referralId'],
        'recipientIntegrationId': json['recipientIntegrationId'] == null ? undefined : json['recipientIntegrationId'],
        'importId': json['importId'] == null ? undefined : json['importId'],
        'reservation': json['reservation'] == null ? undefined : json['reservation'],
        'batchId': json['batchId'] == null ? undefined : json['batchId'],
        'isReservationMandatory': json['isReservationMandatory'] == null ? undefined : json['isReservationMandatory'],
        'implicitlyReserved': json['implicitlyReserved'] == null ? undefined : json['implicitlyReserved'],
        'applicationId': json['applicationId'],
        'applicationName': json['applicationName'],
    };
}

export function CouponWithApplicationToJSON(json: any): CouponWithApplication {
    return CouponWithApplicationToJSONTyped(json, false);
}

export function CouponWithApplicationToJSONTyped(value?: CouponWithApplication | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'id': value['id'],
        'created': value['created'].toISOString(),
        'campaignId': value['campaignId'],
        'value': value['value'],
        'usageLimit': value['usageLimit'],
        'discountLimit': value['discountLimit'],
        'reservationLimit': value['reservationLimit'],
        'startDate': value['startDate'] == null ? value['startDate'] : value['startDate'].toISOString(),
        'expiryDate': value['expiryDate'] == null ? value['expiryDate'] : value['expiryDate'].toISOString(),
        'limits': value['limits'] == null ? undefined : ((value['limits'] as Array<any>).map(LimitConfigToJSON)),
        'usageCounter': value['usageCounter'],
        'discountCounter': value['discountCounter'],
        'discountRemainder': value['discountRemainder'],
        'reservationCounter': value['reservationCounter'],
        'attributes': value['attributes'],
        'referralId': value['referralId'],
        'recipientIntegrationId': value['recipientIntegrationId'],
        'importId': value['importId'],
        'reservation': value['reservation'],
        'batchId': value['batchId'],
        'isReservationMandatory': value['isReservationMandatory'],
        'implicitlyReserved': value['implicitlyReserved'],
        'applicationId': value['applicationId'],
        'applicationName': value['applicationName'],
    };
}

