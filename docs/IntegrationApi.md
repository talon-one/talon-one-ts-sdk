...
async function example() {
  ...
  const body = {
    // NewAudience | body
    newAudience: {"name":"VIP Customers","description":"Customers with high lifetime value and frequent purchases","sandbox":false,"createdIn3rdParty":false},
...
async function example() {
  ...
  const body = {
    ...
    achievements: true,
    // boolean | Set to `true` to include `unlocked` rewards that have not been `used` in the response. (optional)
...
| **unlockedRewards** | `boolean` | Set to &#x60;true&#x60; to include &#x60;unlocked&#x60; rewards that have not been &#x60;used&#x60; in the response. | [Optional] [Defaults to `undefined`] |
...
import type { IntegrationGetAllCampaignsRequest } from 'talon_one_sdk';
...
async function example() {
  ...
  const body = {
    ...
    // Array<string> | Filter by one or more campaign IDs, separated by a comma.  **Note:** If no campaigns are specified, data for all the campaigns in the Application is returned.  (optional)
    campaignIds: ...,
    // Date | Filter results to only include campaigns that start on or after  the specified timestamp.  **Note:**  - It must be an RFC3339 timestamp string.  - You can include a time component in your string, for example, `T23:59:59` to specify the end of the day. The time zone setting considered is `UTC`. If you do not include a time component, a default time value of `T00:00:00` (midnight) in `UTC` is considered.  (optional)
    startAfter: 2013-10-20T19:20:30+01:00,
    // Date | Filter results to only include campaigns that start on or before  the specified timestamp.  **Note:**  - It must be an RFC3339 timestamp string.  - You can include a time component in your string, for example, `T23:59:59` to specify the end of the day. The time zone setting considered is `UTC`. If you do not include a time component, a default time value of `T00:00:00` (midnight) in `UTC` is considered.  (optional)
    startBefore: 2013-10-20T19:20:30+01:00,
    // Date | Filter results to only include campaigns that end on or after  the specified timestamp.  **Note:**  - It must be an RFC3339 timestamp string.  - You can include a time component in your string, for example, `T23:59:59` to specify the end of the day. The time zone setting considered is `UTC`. If you do not include a time component, a default time value of `T00:00:00` (midnight) in `UTC` is considered.  (optional)
    endAfter: 2013-10-20T19:20:30+01:00,
    // Date | Filter results to only include campaigns that end on or before  the specified timestamp.  **Note:**  - It must be an RFC3339 timestamp string.  - You can include a time component in your string, for example, `T23:59:59` to specify the end of the day. The time zone setting considered is `UTC`. If you do not include a time component, a default time value of `T00:00:00` (midnight) in `UTC` is considered.  (optional)
    endBefore: 2013-10-20T19:20:30+01:00,
    // number | Filter results to campaigns linked to the specified store ID. (optional)
    storeId: 789,
    // number | Filter results to campaigns linked to the specified audience ID. (optional)
    audienceId: 789,
  } satisfies IntegrationGetAllCampaignsRequest;

  try {
    const data = await api.integrationGetAllCampaigns(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
| **skip** | `number` | The number of items to skip when paging through large result sets. | [Optional] [Defaults to `undefined`] |
| **campaignIds** | `Array<string>` | Filter by one or more campaign IDs, separated by a comma.  **Note:** If no campaigns are specified, data for all the campaigns in the Application is returned.  | [Optional] |
| **startAfter** | `Date` | Filter results to only include campaigns that start on or after  the specified timestamp.  **Note:**  - It must be an RFC3339 timestamp string.  - You can include a time component in your string, for example, &#x60;T23:59:59&#x60; to specify the end of the day. The time zone setting considered is &#x60;UTC&#x60;. If you do not include a time component, a default time value of &#x60;T00:00:00&#x60; (midnight) in &#x60;UTC&#x60; is considered.  | [Optional] [Defaults to `undefined`] |
...
| **audienceId** | `number` | Filter results to campaigns linked to the specified audience ID. | [Optional] [Defaults to `undefined`] |
...
> IntegrationRewardsCatalog200Response integrationRewardsCatalog(pageSize, skip, pointsFrom, pointsTo, includeFree, loyaltyProgramId, subledgerId, profileIntegrationId, loyaltyCardId)
...
import type { IntegrationRewardsCatalogRequest } from 'talon_one_sdk';
...
async function example() {
  ...
  const body = {
    ...
    // number | The number of items to skip when paging through large result sets. (optional)
    skip: 789,
    // number | Return only rewards whose points required is greater than or equal to this value. (optional)
    pointsFrom: 8.14,
    // number | Return only rewards whose points required is less than or equal to this value. (optional)
    pointsTo: 8.14,
    // boolean | Whether to include rewards that have no `pointsRequired`. These rewards are treated as free and available to all customers.  (optional)
    includeFree: true,
    // number | Return only rewards available in this loyalty program.  (optional)
    loyaltyProgramId: 789,
  ...
  try {
    const data = await api.integrationRewardsCatalog(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
| **pageSize** | `number` | The number of items in the response. | [Optional] [Defaults to `1000`] |
| **skip** | `number` | The number of items to skip when paging through large result sets. | [Optional] [Defaults to `undefined`] |
| **pointsFrom** | `number` | Return only rewards whose points required is greater than or equal to this value. | [Optional] [Defaults to `undefined`] |
| **pointsTo** | `number` | Return only rewards whose points required is less than or equal to this value. | [Optional] [Defaults to `undefined`] |
...
import type { JoinLoyaltyProgramRequest } from 'talon_one_sdk';
...
async function example() {
  ...
  const body = {
    ...
    // string | The integration ID of the customer profile. You can get the `integrationId` of a profile using: - A customer session integration ID with the [Update customer session](https://docs.talon.one/integration-api#tag/Customer-sessions/operation/updateCustomerSessionV2) endpoint. - The Management API with the [List application\'s customers](https://docs.talon.one/management-api#tag/Customer-data/operation/getApplicationCustomers) endpoint. 
    integrationId: integrationId_example,
  } satisfies JoinLoyaltyProgramRequest;

  try {
    const data = await api.joinLoyaltyProgram(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
> ReopenSessionResponse reopenCustomerSession(customerSessionId, idempotencyKey)
...
async function example() {
  ...
  const body = {
    ...
    customerSessionId: customerSessionId_example,
    // string | A unique identifier that enables idempotent processing. Include it to ensure that the request is processed only once, even if you send it several times. (optional)
...
| Name | Type | Description  | Notes |
|------------- | ------------- | ------------- | -------------|
| **customerSessionId** | `string` | The &#x60;integration ID&#x60; of the customer session. You set this ID when you create a customer session.  You can see existing customer session integration IDs in the Campaign Manager\&#39;s **Sessions** menu, or via the [List Application session](https://docs.talon.one/management-api#tag/Customer-data/operation/getApplicationSessions) endpoint.  | [Defaults to `undefined`] |
| **idempotencyKey** | `string` | A unique identifier that enables idempotent processing. Include it to ensure that the request is processed only once, even if you send it several times. | [Optional] [Defaults to `undefined`] |
...
> IntegrationStateV2 returnCartItems(customerSessionId, returnIntegrationRequest, dry, runRuleEngine, idempotencyKey)
...
async function example() {
  ...
  const body = {
    ...
    dry: true,
    // boolean | When set to `true`, reevaluates the updated session after items are returned. Only reevaluates campaigns where `reevaluateOnReturn` is set to `true` and which produced an effect when the session was closed.  (optional)
    runRuleEngine: true,
    // string | A unique identifier that enables idempotent processing. Include it to ensure that the request is processed only once, even if you send it several times. (optional)
    idempotencyKey: idempotencyKey_example,
...
| **runRuleEngine** | `boolean` | When set to &#x60;true&#x60;, reevaluates the updated session after items are returned. Only reevaluates campaigns where &#x60;reevaluateOnReturn&#x60; is set to &#x60;true&#x60; and which produced an effect when the session was closed.  | [Optional] [Defaults to `undefined`] |
| **idempotencyKey** | `string` | A unique identifier that enables idempotent processing. Include it to ensure that the request is processed only once, even if you send it several times. | [Optional] [Defaults to `undefined`] |
...
async function example() {
  ...
  const body = {
    ...
    catalogSyncRequest: {"actions":[{"type":"ADD","payload":{"sku":"SKU1241028","attributes":{"color":"Navy blue","type":"shoes"},"replaceIfExists":true,"price":100,"product":{"name":"sneakers"}}}]},
...
> IntegrationEventV2Response trackEventV2(integrationEventV2Request, silent, dry, forceCompleteEvaluation, idempotencyKey)
...
async function example() {
  ...
  const body = {
    ...
    integrationEventV2Request: {profileId=customer-123, type=newsletter_signup, attributes={SubscriptionType=weekly}, responseContent=[customerProfile, triggeredCampaigns]},
    // string | Possible values: `yes` or `no`. - `yes`: Increases the performance of the API call by returning a 204 response. - `no`: Returns a 200 response that contains the updated customer profiles.  (optional)
    silent: silent_example,
    // boolean | Indicates whether to persist the changes. Changes are ignored when `dry=true`.  (optional)
    dry: true,
...
async function example() {
  ...
  const body = {
    ...
    loyaltyProgramId: 789,
    // string | The identifier of the loyalty card. You can get this ID using the [List loyalty cards](https://docs.talon.one/management-api#tag/Loyalty-cards/operation/getLoyaltyCards) endpoint.  **Important**: The loyalty card ID requires [URL encoding](https://www.w3schools.com/tags//ref_urlencode.asp) if it contains special characters. For example, you must encode `NewCard2026%` as `NewCard2026%25`. 
    loyaltyCardId: loyaltyCardId_example,
    // LoyaltyCardRegistration | body
    loyaltyCardRegistration: ...,
...
import type { UnlockRewardRequest } from 'talon_one_sdk';
...
async function example() {
  ...
  const body = {
    // number | The ID of the reward. You can get the ID with the [List rewards](#tag/Rewards/operation/listRewards) endpoint.
    rewardId: 789,
    // IntegrationUnlockRewardRequest
    integrationUnlockRewardRequest: ...,
    // boolean | When set to `true`, the rule evaluation is performed but no changes are persisted. Use this to preview the outcome of an unlocking. (optional)
    dry: true,
  } satisfies UnlockRewardRequest;

  try {
    const data = await api.unlockReward(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
| Name | Type | Description  | Notes |
|------------- | ------------- | ------------- | -------------|
| **rewardId** | `number` | The ID of the reward. You can get the ID with the [List rewards](#tag/Rewards/operation/listRewards) endpoint. | [Defaults to `undefined`] |
| **integrationUnlockRewardRequest** | [IntegrationUnlockRewardRequest](IntegrationUnlockRewardRequest.md) |  | |
| **dry** | `boolean` | When set to &#x60;true&#x60;, the rule evaluation is performed but no changes are persisted. Use this to preview the outcome of an unlocking. | [Optional] [Defaults to `undefined`] |
...
> CustomerProfileIntegrationResponseV2 updateCustomerProfileV2(integrationId, customerProfileIntegrationRequestV2, runRuleEngine, dry, idempotencyKey)
...
async function example() {
  ...
  const body = {
    ...
    customerProfileIntegrationRequestV2: {"attributes":{"Name":"Chris Taylor","Email":"chris.taylor@example.com","SignupDate":"2026-01-21T15:04:05+07:00","PaymentMethod":"Bank transfer"}},
...
> MultipleCustomerProfileIntegrationResponseV2 updateCustomerProfilesV2(multipleCustomerProfileIntegrationRequest, silent, idempotencyKey)
...
> IntegrationStateV2 updateCustomerSessionV2(customerSessionId, integrationRequest, dry, now, idempotencyKey)
...
async function example() {
  ...
  const body = {
    ...
    integrationRequest: {"customerSession":{"profileId":"382370BKDB946","cartItems":[{"name":"Wireless Bluetooth Headphones","sku":"AUDIO-WH350","quantity":1,"price":79.99,"weight":310},{"name":"USB-C Charging Cable","sku":"CABLE-USBC-2M","quantity":2,"price":15.99,"weight":40}]},"responseContent":["customerSession","customerProfile"]},
