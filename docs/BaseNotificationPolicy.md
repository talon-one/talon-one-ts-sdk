
# BaseNotificationPolicy

Indicates which notification properties to apply.

## Properties

Name | Type
------------ | -------------
`name` | string
`triggers` | [Array&lt;TierWillDowngradeNotificationTrigger&gt;](TierWillDowngradeNotificationTrigger.md)
`batchingEnabled` | boolean
`batchSize` | number
`scopes` | Array&lt;string&gt;
`includeData` | boolean
`aheadOfDaysTrigger` | number

## Example

```typescript
import type { BaseNotificationPolicy } from 'talon_one_sdk'

// TODO: Update the object below with actual values
const example = {
  "name": Notification to Google,
  "triggers": null,
  "batchingEnabled": false,
  "batchSize": 1000,
  "scopes": null,
  "includeData": true,
  "aheadOfDaysTrigger": null,
} satisfies BaseNotificationPolicy

console.log(example)

// Convert the instance to a JSON string
const exampleJSON: string = JSON.stringify(example)
console.log(exampleJSON)

// Parse the JSON string back to an object
const exampleParsed = JSON.parse(exampleJSON) as BaseNotificationPolicy
console.log(exampleParsed)
```

[[Back to top]](#) [[Back to API list]](../README.md#api-endpoints) [[Back to Model list]](../README.md#models) [[Back to README]](../README.md)


